...
void set_sample_prog(uint256_t program[]) {
  ...
  // set bytecode //
  change_uint256(&program[0], 0x1110203040500700, 0x01, 0x02, 0x03);
  change_uint256(&program[1], 0x04, 0x05, 0x06, 0x07);
  change_uint256(&program[2], 0x09, 0x0A, 0x0B, 0x0C);
  change_uint256(&program[3], 0x0E, 0x0F, 0x10, 0x11);

  // set bytecode for last index
  change_uint256(&program[MAX_BYTECODE_LEN - 1], 0x13, 0x14, 0x15, 0x16);
}
...
void set_memory(uint256_t memory[]) {
  ...
  // set memory //
  change_uint256(&memory[0], 0x1110203040500700, 0x01, 0x02, 0x03);
  change_uint256(&memory[1], 0x04, 0x05, 0x06, 0x07);
  change_uint256(&memory[2], 0x09, 0x0A, 0x0B, 0x0C);
  change_uint256(&memory[3], 0x0E, 0x0F, 0x10, 0x11);

  // set memory for last index
  change_uint256(&memory[MAX_MEMORY_LEN - 1], 0x13, 0x14, 0x15, 0x16);
}
